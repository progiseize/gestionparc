...
if ($action == 'delete'):
    $error = 0;
    if(GETPOST('token') != $_SESSION['token']): $error++; setEventMessages($langs->trans('SecurityTokenHasExpiredSoActionHasBeenCanceledPleaseRetry'), null, 'warnings'); endif;
    if(!$error): echo $form->formconfirm($_SERVER['PHP_SELF'].'?socid='.$socid.'&itemid='.GETPOST('itemid').'&parcid='.GETPOST('parcid'),$langs->trans('gp_confirmDeleteTitle'),$langs->trans('gp_confirmDelete'),'confirm_delete','','',1,0,500,0); endif;
endif;
...
<div id="pgsz-option" class="tabBar pgsz-theme-<?php echo $conf->theme; ?>">
    ...
    <div class="fichecenter">
        ...
        <table class="border centpercent tableforfield">
            ...
            <?php endif; ?>
            <?php $isCookieParc = GestionParcCheckCookieForSocid('gestionparc_empty_views',$socid);?>
            <tr>
                <td><?php echo $langs->trans('gp_showEmptyParcs'); ?></td>
                <td>
                    <?php $newValCookie = ($isCookieParc)?'0':'1'; ?>
                    <form enctype="multipart/form-data" action="<?php print $_SERVER["PHP_SELF"]; ?>?socid=<?php echo $societe->id; ?>" method="POST">
                        <input type="hidden" name="action" value="set_cookie_parc">
                        <input type="hidden" name="new_cookie" value="<?php echo $newValCookie; ?>">
                        <input type="checkbox" name="view_empty_parc" <?php if($isCookieParc): echo 'checked="checked"'; endif; ?>>
                    </form>
                </td>
            </tr>
            ...
            <?php endif; ?>
            <?php if($conf->global->MAIN_MODULE_GESTIONPARC_USEVERIF && $nb_totalinparc): ?>
            <tr>

...
class GestionParcVerif {
	...
	public function closeVerif($rowid,$socid,$description,$duree){
        ...
        $modele_desc = $conf->global->MAIN_MODULE_GESTIONPARC_VERIFMODEL;
		...
		// CONTENU 
        $lineverif_desc = ''; $i = 0;
        foreach($list_parctypes as $parctype_key => $parctype_infos): $i++;

        	$list_parcFields = $gestionparc->list_parcFields($parctype_key);
            $parc_lines = $gestionparc->getSocParcContent($socid,$parctype_infos['key']);
            ...
            if($nb_parclines > 0):
	            ...
	            //
	            foreach($parc_lines as $parcline):
	            	if($parcline->verif):$verified_lines++;endif;
	            endforeach;
        ...
        $moreparams = array('filename' => 'yopdf',);
        $intervention->generateDocument($this->model_pdf,$langs,0,0,0,$moreparams); // New Doc en cours de dev
